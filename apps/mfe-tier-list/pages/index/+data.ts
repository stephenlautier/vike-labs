...
export async function data(): Promise<Data> {
	...
	if (!tiersRes.ok) throw new Error(`Failed to load tier list: HTTP ${tiersRes.status}`);
	if (!championsRes.ok) throw new Error(`Failed to load champions: HTTP ${championsRes.status}`);
